...
public interface AppCache {
  ...
  /**
   * 累计数目 效率较高的 计数器 如需清零，按照普通key 移除即可
   *
   * @param key   key值
   * @param value 去重统计值
   * @return 计数器结果
   ...
   * 计数器结果
   * <p>
   * 效率较高的 计数器 统计返回 如需清零，按照普通key 移除即可
   *
   * @param key 计数器key
   * @return 计数器结果
   */
  ...
  /**
   * redis 计数器 累加 注：到达liveTime之后，该次增加取消，即自动-1，而不是redis值为空
   *
   ...
   * @return 获取满足条件的集合
   */
  Set<ZSetOperations.TypedTuple<Object>> reverseRangeWithScores(String sortedSetName, Integer start,
      Integer end);
   ...
   * @return 获取满足条件的集合
   */
  Set<ZSetOperations.TypedTuple<Object>> reverseRangeWithScores(String sortedSetName,
      Integer count);
