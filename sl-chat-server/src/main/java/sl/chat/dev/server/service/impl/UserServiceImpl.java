...
import sl.chat.dev.common.cache.CachePrefix;
...
import sl.chat.dev.server.util.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public void register(RegisterReq dto) {
        ...
        String userChatCode = "sl" + CodeUtils.getRandomCode(10);
        user.setUserChatCode(userChatCode);
        user.setPassword(passwordEncoder.encode(user.getPassword()));
        user.setSignature("我就是我，不一样的烟火");
        user.setHeadImageThumb("http://127.0.0.1:9000/box-im/image/20231130/xxx.jpeg");
        this.save(user);
    ...
    @Override
    public List<UserVO> findUserByCode(String userChatCode) {
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda()
                .eq(User::getAccountType, UserEnum.AccountType.Plain.getCode())
                .eq(User::getUserChatCode, userChatCode)
                .last("limit 20");
        List<User> users = this.list(queryWrapper);
        List<UserVO> vos = users.stream().map(u -> {
            UserVO vo = BeanUtils.copyProperties(u, UserVO.class);
