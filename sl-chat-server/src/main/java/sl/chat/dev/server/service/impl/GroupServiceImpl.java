...
import sl.chat.dev.common.core.enums.CommonEnums;
...
public class GroupServiceImpl extends ServiceImpl<GroupMapper, Group> implements IGroupService {
  ...
  public GroupVO modifyGroup(GroupVO vo) {
    ...
    member.setAliasName(
        StringUtils.isEmpty(vo.getAliasName()) ? session.getNickName() : vo.getAliasName());
  ...
  public void quitGroup(Long groupId) {
    ...
    iChatSessionService.del(
        ChatSessionUpdateReq.builder().chatType(ChatType.GROUP).targetId(groupId).build());
  ...
  public void kickGroup(Long groupId, Long userId) {
    ...
    iChatSessionService
        .del(
            ChatSessionUpdateReq.builder().chatType(ChatType.GROUP).userId(userId).targetId(groupId)
                .build());
  ...
  public List<GroupVO> findGroups() {
    ...
    // 拉取群列表
    List<Long> ids = groupMembers.stream().map((gm -> gm.getGroupId()))
    ...
    List<GroupVO> vos = groups.stream().map(g -> {
      ...
      GroupMember member = groupMembers.stream().filter(m -> g.getId().equals(m.getGroupId()))
          .findFirst().get();
      vo.setAliasName(member.getAliasName());
      vo.setRemark(member.getRemark());
      return vo;
  ...
  public void invite(GroupInviteReq vo) {
    ...
    if (vo.getFriendIds().size() + size > Constant.MAX_GROUP_MEMBER) {
      throw new GlobalException(ResultCode.PROGRAM_ERROR,
          "群聊人数不能大于" + Constant.MAX_GROUP_MEMBER + "人");
    }
    ...
    if (vo.getUserId() != null) {
      ...
      List<Long> memberIds = members.stream().map(GroupMember::getUserId)
          .collect(Collectors.toList());
