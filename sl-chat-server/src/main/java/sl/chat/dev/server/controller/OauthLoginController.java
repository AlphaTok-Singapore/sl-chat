...
import sl.chat.dev.server.common.vo.user.LoginResp;
import sl.chat.dev.server.config.properties.AppConfigInfo;
import sl.chat.dev.server.service.IUserService;
import sl.chat.dev.server.util.OauthLoginUtils;
import sl.chat.dev.server.util.SecurityUtils;
  ...
  public void callBack(@PathVariable String type, AuthCallback callback,
    ...
    try {
      ...
      if (code == 2000) {
        ...
        httpServletResponse
            .sendRedirect(
                appConfigInfo.getAuth2().getLoginRedirectUri() + "?state=1&oAuthInfo=" + oAuthInfo);
        return;
      }
    ...
    httpServletResponse.sendRedirect(
        appConfigInfo.getAuth2().getLoginRedirectUri() + "?state=0&errorMsg="
            + SecurityUtils.base64Obj(errorMsg));
  }
