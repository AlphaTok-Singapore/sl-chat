...
import sl.chat.dev.server.config.properties.AppConfigInfo;
...
public class ConnectorNodeListener implements ApplicationListener<ApplicationStartedEvent> {
  ...
  public void onApplicationEvent(ApplicationStartedEvent applicationStartedEvent) {
    ...
    for (NetProtocolEnum protocolEnum : NetProtocolEnum.values()) {
      ...
      // continue;
      // }
      // List<NodeContainer.WNode> nodeList = paths.stream().map(e->{return
      // convertZkNodeToWNote(e,protocolEnum);})
      // .filter(Objects::nonNull).collect(Collectors.toList());
      // this.addNode(protocolEnum,nodeList);
    }
  }

  private NodeContainer.WNode convertZkNodeToWNote(ChildData childData, String parentPath,
      NetProtocolEnum protocolEnum) {
  ...
  private void childrenCacheListener(String listenerPath, NetProtocolEnum netProtocolEnum)
      throws Exception {
      ...
      public void childEvent(CuratorFramework client, PathChildrenCacheEvent event)
        ...
        if (childData != null) {
          if (PathChildrenCacheEvent.Type.CHILD_ADDED == event.getType()) {
            NodeContainer.WNode wNode = convertZkNodeToWNote(childData, listenerPath,
                netProtocolEnum);
            if (wNode != null) {
